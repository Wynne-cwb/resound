...
- **录音库** — 搜索、文件夹分组、⌘F 查找替换（修转录错字）、逐句点跳播放、说话人试听；反复改同一个错词会**自动建议加入词表**，一键确认。新录音还会**智能推算文件夹**——LLM 读摘要挑最合适的现有文件夹（或提议新建），以列表角标给出，你确认或忽略（不点头绝不擅自移动）。
