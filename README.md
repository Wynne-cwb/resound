...
- **Meeting recording** — One click captures your mic + the other side of a Google Meet call (dual-track capture via ScreenCaptureKit; both tracks are aligned by true start time, mixed for archival, and **kept as separate tracks** — each transcribed independently and merged with dedup, so the same speech arriving over two paths in a hybrid in-room + online meeting doesn't smear the transcript). When a Meet call is detected it pops a screen-level prompt; you can set it to **auto-start on meeting begin / auto-stop on meeting end** (or surface a one-click confirmation panel), no manual steps needed. While recording, a small **draggable on-screen indicator** (pulsing dot + elapsed time + stop button) floats above other apps so you always know you're recording and can stop in one click — even when the main window is hidden. Toggle it in **Settings › General › Recording indicator**.
- **Transcription** — Three backends: **MOSS cloud (recommended)**, online whisper, and local WhisperKit. MOSS ([MOSS-Transcribe-Diarize](https://github.com/OpenMOSS/MOSS-Transcribe-Diarize), Apache-2.0) is an end-to-end joint "transcription + diarization" model — who-said-what comes out in a single pass, with speaker attribution notably stronger than stitched pipelines. Settings offers **one-click deployment to your own Modal account** (effectively free within the monthly credit), with automatic fallback to whisper on failure. Long recordings (>30 min) are **automatically chunked and transcribed in parallel** then stitched back together — the 0.9B model's output format degrades on hour-plus inputs, so chunking keeps both quality and wall-clock time in check. The whisper path defaults to online `whisper-large-v3-turbo` (fast), with local WhisperKit as an offline fallback. Before upload, silero VAD **trims long silences/noise** (cutting whisper's "thanks for watching"-style hallucinations and saving tokens, with timestamps mapped back to the original timeline), then applies **windowed adaptive loudness normalization** (rescues both all-quiet recordings and quiet-start/loud-rest ones). Meeting recordings **transcribe each track independently** and merge by timestamp with dedup — in hybrid in-room + online meetings, the same speech arriving over two paths no longer smears the transcript. After transcription: Traditional→Simplified normalization + glossary correction + **LLM proofreading** (fixing homophone typos and misheard English proper nouns).
- **Speaker diarization** — Sortformer neural segmentation (running on the Apple Neural Engine) → silero VAD to drop silence → CAM++ voiceprints → match against the enrollment library for real names; clusters are merged when voiceprints are close, with naming kept mutually exclusive to prevent mismatches. Name a voice once and its voiceprint is remembered, recognized automatically across recordings — the more you label, the more accurate it gets. **Named speakers are auto-added to the glossary as transcription bias terms** (names come up often in meetings, so biasing improves their spelling).
...
- **Search & Q&A** — FTS5 keyword + vector recall + RRF fusion + LLM rerank + synthesis; answers come **with citations and dates**. A planner reads each question and routes it to the right shape: pinpoint facts, **digest** ("what did we cover this month"), **timeline** ("how did the migration strategy evolve"), or **compare** ("this week's vs last week's 1-on-1"). Filters compose freely — by time, **by speaker** ("what have Jerry and I discussed"), or by source — and **recency-aware** queries ("the latest plan") favor the most recent discussions. Broad reviews retrieve across your whole history (map-reduce when large), not just a handful of snippets; when a filter matches nothing it **relaxes automatically** instead of dead-ending. Citations are **source-aware** — a single answer can mix 🎙️ recordings and 📄 documents, each clickable (recordings jump to the timestamp, documents open with the cited passage highlighted). You can also **ask about a single recording** (retrieval also pulls in its **linked documents**, so the answer reflects the attached PRD/agenda/material) or **a single document** (scoped strictly to that document).
- **Documents** — Import external material alongside your recordings: meeting PRDs, minutes, compliance docs. Rich formats are parsed into searchable text using native macOS frameworks (zero dependencies): **PDF** (text layer + heading inference, with **OCR fallback for scanned PDFs**), **Word (.docx)**, **PowerPoint (.pptx)**, **HTML**, **images** (Vision OCR, Chinese + English), plus Markdown / plain text. For PDFs and images, the messy extracted text is then **auto-reformatted into clean, readable Markdown by an LLM** (semantics strictly preserved — merge broken lines, drop repeated headers/footers, rebuild tables). The real original file is archived alongside the extracted text. Each document is indexed into the same knowledge base, so it **joins recordings in global Q&A**. Documents and recordings can be **linked both ways** (manage from either side), and each document has its own metadata, tags, and "Ask this document" tab. On import, a document with no tags gets a **smart tag suggestion** (1-2 core tags, reusing existing tags where it can), shown as a list-row badge you accept or ignore.
- **Recording library** — Search, folder grouping, ⌘F find & replace (to fix transcription typos), click-to-jump sentence playback, and speaker preview; repeatedly fixing the same wrong word **auto-suggests adding it to the glossary** with one-click confirm. New recordings also get a **smart folder suggestion** — the LLM reads the summary and picks the best-fitting existing folder (or proposes a new one), shown as a list-row badge you confirm or ignore (nothing is moved without your OK). You can also **merge recordings** — multi-select 2 or more, concatenate them into one new recording in creation-time order and fully re-transcribe (the new title is AI-suggested and editable); the originals are moved to an archive (recoverable, not deleted).
- **External knowledge via MCP (in)** — Connect Notion, Jira / Confluence, or any MCP server (built-in OAuth 2.1 + PKCE + dynamic client registration, or API token; tokens live in the Keychain). Then **paste a document link onto a recording** — Resound fetches the content through MCP, ingests it into the same knowledge base, and it joins recordings + local docs in search, Q&A, and meeting notes. Smart degradation when a link can't be fully fetched: unconnected source → prompts you to connect; unknown / intranet URL or no-permission → saved as a clickable **link-only** reference. Custom sources can be remote (HTTP) or **local stdio** (a subprocess like `npx -y @org/mcp-server`, env vars stay on-device). Configure in **Settings › External MCP**.
- **Resound as an MCP server (out)** — Expose your meeting knowledge base as a local stdio MCP server so coding agents (**Claude Code / Codex**) can query your meetings and documents — entirely on-device. One-click install writes the agent config for you (manual command provided as fallback); a global **content policy** (full / summary+link / link-only) controls how much of *external-source* documents is shared (your own recordings are always served in full). Configure in **Settings › Resound MCP**.
...
| `TRANSCRIBE_BACKEND` | `moss` = prefer MOSS cloud transcription (falls back to whisper on failure) |
| `MOSS_SUBMIT_URL` / `MOSS_RESULT_URL` / `MOSS_API_KEY` | MOSS service endpoints and key (written automatically by in-app one-click deploy) |
...
| `normalize-audio` | Windowed adaptive loudness normalization on an audio file (debug; same implementation as the pre-transcription step) |
| `recover-meeting` | Stream-mix leftover mic/sys temp tracks (memory-safe) to salvage a meeting lost to a crash |
| `sync-speaker-names` | Add registered speaker names to the vault glossary as bias terms (also happens automatically when you name a speaker) |
...
| `suggest-folder` / `suggest-tags` | Print the smart folder / tag suggestion for a recording / document (debug; for tuning the classifier prompt) |
...
| `index-prune` | Remove index entries for recordings no longer in the vault (run after archiving/deleting dirs by hand) |
...
| `mcp serve` | Run Resound as a stdio MCP server so coding agents (Claude Code / Codex) can search your meetings & documents (usually launched by the agent, not by hand) |
| `mcp selftest` | Headless check of the MCP server tools (search/get/list) and install commands |
| `mcp sources` / `mcp fetch` / `mcp sync` | Inspect external MCP sources, resolve a pasted link, re-sync an external doc (debug for the external-sources feature) |
| `doctor` | Self-check key dependencies such as sqlite-vec |

## How it works

```
record ─► VAD gating (trim silence/noise) ─► transcribe (MOSS cloud, joint transcription+diarization / online whisper / local WhisperKit) ─► zh normalization + glossary correction + LLM proofread
   └─► diarization (MOSS: labels → CAM++ voiceprint naming; whisper: Sortformer segmentation @ANE → VAD → CAM++ voiceprints → enrollment match)
                          │
