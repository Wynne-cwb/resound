...
- **Recording library** — Search, folder grouping, ⌘F find & replace (to fix transcription typos), click-to-jump sentence playback, and speaker preview; repeatedly fixing the same wrong word **auto-suggests adding it to the glossary** with one-click confirm. New recordings also get a **smart folder suggestion** — the LLM reads the summary and picks the best-fitting existing folder (or proposes a new one), shown as a list-row badge you confirm or ignore (nothing is moved without your OK).
