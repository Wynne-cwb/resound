...
**模块 A 定论**：
- **消费形态=绑定时拉取入库**（走现有文档管线 chunk→embed→SQLite），检索/Ask 不变（本地/快/可引用/离线）。**否决**查询时实时 agentic 拉取（要把 App 改成托管 agent loop 的 MCP 客户端，慢/引用难/破坏离线/与固定 retrieve→synthesize 管线冲突）。
- **新鲜度=版本戳判变更**：Notion `last_edited_time`/Jira `updated`/Drive `modifiedTime` 这种廉价信号——戳没变用缓存，变了重拉。**不每查都判**，加 TTL + 显式「同步最新」按钮（尤其生成纪要前）。重拉很便宜：`embedding_cache`（上轮做的）让只有真正变了的 chunk 重嵌。冷门源不给戳→退化为定时重拉/仅手动刷新。
- **接入架构=通用 MCP 客户端**（MCP swift-sdk + HTTP/SSE + JSON-RPC）。**否决**每家专写 REST 适配器（做不到"接任意自定义源"，且要实现 Notion/Jira/Google 三套 OAuth）。**走 MCP 反而收敛鉴权**：托管 MCP server 自己搞定到后端的上游鉴权，Resound 只对 MCP server 做一种 OAuth。
- **鉴权可行**：OAuth 2.1 + PKCE 经 `ASWebAuthenticationSession`（AuthenticationServices），回调走自定义 scheme/loopback，token 存 Keychain，401 自动 refresh。**支持 DCR（动态客户端注册）的源（Notion/Atlassian 托管 MCP）真一键**——临场注册无需预建 OAuth app；不支持 DCR 的退化为"贴一次 client_id 后一键"。
- **内置预设**：Notion / Jira·Confluence / Google Workspace / Figma + 自定义任意 MCP server。
- **绑定机制 MVP=粘贴链接**（复用现有"关联文档"机制，零检索 UI）；后续可加应用内 MCP 搜索、自动推荐（像智能推算 tag）。
- **URL 路由 + 两级降级**（治"贴的 URL 不属任何已连 MCP/内网/不支持"）：按 host 匹配已连来源→①命中已连=拉全文成**「已索引外部文档」**（进检索带引用）；②像已知源但未连=引导一键连接；③未知/内网/拉空=存成**「仅链接引用」**（可点书签，不进检索）；④能连但无权限=同③+权限提示。数据模型复用现有文档模块（`source_kind=notion/jira/…`）关联录音。

**模块 B 定论**：
- **承载=`resound mcp` stdio 子命令**，被 agent 拉起为子进程，只读共享 `index.sqlite`，GUI 开不开都不影响、零 IPC。
- **暴露粒度=检索原语**（非高层 ask 黑盒）：`search_meetings(query,filters?,topK?)`（复用 hybrid/RRF/filters 时间·说话人·来源·文件夹）+ `get_recording(id,include:transcript|summary|both)` + `get_document(id)`（+可选 `list_recordings`）。coding agent 用自己的模型综合。
- **一键安装=代写配置**：shell out 调两家官方命令 `claude mcp add resound -- <path> mcp` / `codex mcp add resound -- <path> mcp`（Codex 现也有 `codex mcp add`），检测 CLI 在否点亮按钮，命令展出兜底。
- **外部文档传输方式（全局开关）**：服务 coding agent 时,**已索引外部文档**(模块 A 拉进来的 Notion/Jira/…)怎么呈现可配——`完整内容`(塞全文,适合 agent 没接该源/要自包含/离线) / `仅链接`(只给 URL+元信息,适合 agent 自己接了该源、要实时+自己的权限) / `片段+链接`(命中片段/摘要+URL,**默认**,agent 自己决定要不要拉全文)。**作用范围**：只管外部文档、只在 MCP 出口生效——**录音/转录/摘要永远返回内容**(agent 别处拿不到)、「仅链接引用」天然只有 URL、**不影响模块 A 的本地索引**(Resound 自己 Ask/纪要照常用全文)。`search_meetings` 命中片段无论如何都返回。**粒度=全局一个开关**(否决每来源可配/agent 请求时参数控——前者 UI 重、后者用户端不可控)。

**设计稿验收 + spec/计划（2026-06-26 同日稍后）**：Claude Design handoff `Resound.dc.html` 整份读过——忠实且优秀，覆盖全部讨论状态，另补 API Token 认证/授权过期重连/每来源同步态/手动命令兜底/URL 四路降级。**范围拍板**：①**不做 Cursor**（无官方 `cursor mcp add`，靠写 `~/.cursor/mcp.json`，本期略）；②**本地 stdio 自定义来源纳入 v1**（add-source modal 的「本地·stdio 命令」分支，Resound 起子进程）。**SDK 核实**：官方 `modelcontextprotocol/swift-sdk`（product MCP, from 0.11.0）——`Server`+`StdioTransport`（模块B✓）、`Client`+`HTTPClientTransport`（远程，含SSE）+本地子进程stdio（模块A✓）；HTTP **服务器**端不成熟(issue #165)但我们服务器只走 stdio 不受影响；**OAuth 自实现**（ASWebAuthenticationSession+PKCE+DCR+Keychain，token 注入 HTTP 头）。**实现顺序按风险**（非模块先后）：Wave1 模块B服务器(最小最稳) → Wave2 模块A后端(远程优先,CLI验绿) → Wave3 App UI → Wave4 本地stdio。spec/计划 = [specs/2026-06-26-mcp-bidirectional-design.md](superpowers/specs/2026-06-26-mcp-bidirectional-design.md) / [plans/2026-06-26-mcp-bidirectional-plan.md](superpowers/plans/2026-06-26-mcp-bidirectional-plan.md)。**最大不确定点**=每来源「取正文/取版本戳」工具映射（内置写适配器，通用靠 MCP Resources）。

**Wave 0 踩坑（2026-06-26）：swift-sdk 版本必须锁 `<0.10.2`**。`from:"0.11.0"` 解析到 0.12.1，编译在 swift-sdk **内部** `HTTPClientTransport.swift` 报 `withThrowingTaskGroup` 缺 `of:`——因 0.10.2+ 的 `swift-tools-version` 跳到 **6.1**，用了 6.1 才有的 `withThrowingTaskGroup {…}`（无 `of:`，靠 body 推断子结果）重载；当前机器工具链是 **Swift 6.0.3**（`swift-tools 6.0`）编不过。**0.10.1 是最后一个 swift-tools 6.0 兼容版**（无该写法）。Package.swift 锁 `"0.9.0"..<"0.10.2"`→解析 0.10.1，`swift build` exit 0。验证过 0.10.1 仍有 Server/StdioTransport/Client/HTTPClientTransport（够用）。**教训**：引第三方 SwiftPM 包先核对它的 swift-tools-version vs 本机 `swift --version`，新包常领先工具链。要用最新 SDK 得先升 Xcode/工具链到 6.1+（用户机器、暂不动）。

**Wave 1 ✅ 模块B服务器（2026-06-26）**：`resound mcp serve`（[MCPServer.swift](../Sources/ResoundCore/MCPServer.swift)）= swift-sdk `Server`+`StdioTransport`，4 工具 search_meetings/get_recording/get_document/list_recordings（snake_case，SDK 要求），复用 `IndexPipeline.search`/`Index.Filters`/`listRecordings`/`loadTranscript`。工具逻辑抽成可独立调的方法，CallTool handler 与 `runSelftest` 共用（后者免传输无头验）。内容策略 [MCPServerSettings.swift](../Sources/ResoundCore/MCPServerSettings.swift)（`mcp-server.json` full/link/summary，默认 summary；`parseExternalBlock` 读 document.yaml `external:` 块——为 Wave2 就位）。安装 [MCPInstall.swift](../Sources/ResoundCore/MCPInstall.swift)：`claude/codex mcp add resound -- <path> mcp serve`，`command -v` 检测，登录 shell 跑（拿用户 PATH）。**`mcp selftest` 实跑全绿**：4 工具返真实数据、Claude Code+Codex 都检测到、serve 启动 EOF 干净退出不崩。

**Wave 2 ✅ 模块A后端编译+离线验（2026-06-26）**：[MCPSource.swift](../Sources/ResoundCore/MCPSource.swift)（来源模型+4 内置预设+`mcp-sources.json` 存储〔load 时内置∪磁盘，host/url 以预设为准防漂移〕+host 后缀匹配）、[MCPClient.swift](../Sources/ResoundCore/MCPClient.swift)（`MCPClientSession` actor 包 swift-sdk `Client`+`HTTPClientTransport`，Bearer 经 **`requestModifier`** 注入〔SDK 的官方注入点〕；`SourceAdapter` 通用取正文=先 `readResource(uri:)` 后找 fetch 类工具调；`ExternalLinkResolver` host→来源→四路 imported/unconnected/unknown/noPermission）、[MCPOAuth.swift](../Sources/ResoundCore/MCPOAuth.swift)（PKCE〔SecRandom+SHA256 base64url〕、DCR 注册、authorizeURL 构造、code↔token、refresh、Keychain `MCPTokenStore`、`validAccessToken` 过期自动续；**交互 ASWebAuthenticationSession 留 Wave 3**）、[MCPIngest.swift](../Sources/ResoundCore/MCPIngest.swift)（imported→DocumentStore.importDocument(external块)+indexDocument；link-only 不索引；resync 重取重建）。DocumentManifest 加 `external` 块（yaml 读写 + updateManifest/resync 保留）。CLI `mcp sources/fetch/sync`。**离线验过**：sources 列 4 预设、fetch 路由 Notion未连→unconnected/内网→unknown。**未验（需用户账号）**：真 OAuth 授权、连上来源后实取正文、每来源真实工具名（通用适配器对支持 Resources 的 server 即可用，内置来源具体工具名待实连时按各家最新文档细化）。**剩 Wave 3 App UI + Wave 4 本地 stdio。**

**Wave 3 ✅ App UI + Wave 4 ✅ 本地 stdio（编译+打包通过，待用户经 UI 实测）（2026-06-26）**：
- **Wave 4 先于 Wave 3 接通**（被 Wave 3 的添加自定义源「本地」分支依赖）：[MCPStdioProcessTransport.swift](../Sources/ResoundCore/MCPStdioProcessTransport.swift)=符合 swift-sdk `Transport` 协议（actor，`logger/connect/disconnect/send/receive`）的子进程传输。swift-sdk 自带 `StdioTransport` 读写**本进程** stdin/stdout（用于「我作 server」），反过来作 client 连本地子进程得自实现：经 `/bin/zsh -lc` 起 `command args`（拿用户 PATH 找 npx/homebrew）、`Pipe` 接子进程 stdin/stdout/stderr、`readabilityHandler` 读 stdout 按换行切 JSON-RPC 喂 `AsyncThrowingStream`、`send` 写 stdin+换行。`MCPClientSession` 泛化成持 `any Transport`，加 `init(command:args:env:)` + `static make(for source:bearer:)` 按 `transport` 远程/本地分流；`ExternalLinkResolver`/`MCPIngest.resync` 改走 `make`。
- **Wave 3 App UI**：[MCPModel.swift](../Sources/ResoundApp/MCPModel.swift)（@MainActor，来源/OAuth/添加源/粘贴链接/服务/内容策略/安装全状态）；OAuth 交互=`ASWebAuthenticationSession`（`callbackURLScheme:"resound"`，回调取 code 校验 state→`MCPOAuth.exchange`→Keychain），`WebAuthPresenter` 提供窗口锚点，DCR 无 client_id 时临场注册。粘贴链接四路接 `ExternalLinkResolver`，imported→`MCPIngest.ingestImported`，其余→`saveLinkOnly`/提示连接。两设置区 [MCPSettingsViews.swift](../Sources/ResoundApp/MCPSettingsViews.swift) + 三模态 [MCPModals.swift](../Sources/ResoundApp/MCPModals.swift)。LibraryView「相关文档」按 `DocumentSummary.external` 拆本地/外部行（新增字段）。
- **⚠️ 踩坑：随包 CLI 不能叫 `resound`**。`bundle-app.sh` 初版把 CLI 拷成 `Contents/MacOS/resound`——**macOS 文件系统大小写不敏感**，`resound` 与 App 主可执行 `Resound` 同名 → cp 直接**覆盖掉 App 二进制**（开 App 会跑成 CLI）。实测确认（bundle 内 38MB=CLI 而非 49MB=App）。改名 `resound-cli`，`appResoundPath` 同步找 `resound-cli`。一键安装命令写 `<app>/Contents/MacOS/resound-cli mcp serve`。
- **打包**：`bundle-app.sh` 加 `swift build --product resound` 并拷 `resound-cli` 入包；App 内 `MCPInstall.appResoundPath(bundleMacOSDir:)` 优先包内 `resound-cli` 回退 `which resound`。Package.swift 加 swift-log 直依赖（自定义 Transport 的 `logger: Logging.Logger` 需要；swift-sdk 已传递依赖）。
- **服务开关语义（v1 简化）**：App「Resound MCP」开关写 `mcp-server.json.enabled`（偏好/展示），但实际暴露由「是否 `claude mcp add` 安装」决定；`mcp serve` 当前不因 enabled=false 拒服务（避免 agent 端静默失效）。一键装/卸才是真开关。
- **待用户经 UI 实测**：①一键装 Claude Code/Codex→进真 agent 查；②连一个来源走 OAuth 浏览器授权；③录音「关联链接」粘 URL→取回入库→相关文档外部行→Ask 引用；④四路提示；⑤内容策略切换；⑥本地 stdio 源（`npx -y @notionhq/notion-mcp-server`）连+取。**OAuth 浏览器授权 + 真取回需用户账号，今晚无法验。**

**来源品牌图标（2026-06-27）**：用户提供 5 个官方 logo SVG。**NSImage 原生支持 SVG**（`_NSSVGImageRep`，矢量），故打包进 App 资源（`Sources/ResoundApp/Resources/MCPIcons/{notion,atlassian,google,figma,mcp}.svg`，Package.swift ResoundApp 加 `resources:[.copy(...)]`，经 `Bundle.module` 读——与 ResoundCore TSCharacters 同机制，bundle-app.sh 已拷 `*.bundle` 入 Contents/Resources）。新增 [SourceIcon.swift](../Sources/ResoundApp/SourceIcon.swift)：彩色 logo（Jira/Google/Figma）`.renderingMode(.original)` 原样、黑白 logo（Notion/自定义=通用 MCP 标）`.template` 随主题着色，取不到回退字母。替换设置来源卡 / 三模态 / 录音外部文档行的字母占位块；删掉各处旧 `monoTile` + `MCPModel.sourceMono`。

**内置来源调研定型：移除 Figma、Google 改手动凭证 + loopback（2026-06-29）**：用户反馈 Figma/Google 连不上，先调研后改。**结论**：
- **Figma**（`mcp.figma.com/mcp`）真有托管 OAuth MCP，但**对第三方客户端封闭**——MCP 访问靠服务端 `mcp:connect` scope 控，**不开放给普通第三方 OAuth 应用**（仅 VS Code/Cursor/Claude Code 等审批过的目录客户端，新客户端排 waitlist）。即便走通 DCR+OAuth 也会被服务端拒。→ **移除 Figma 预设**（保留 `.figma` 枚举 case + 图标，避免旧 mcp-sources.json 解码失败；只从 `builtins()` 删，UI 不再出现）。
- **Google Workspace** 官方远程 MCP **真实存在但与 Notion/Atlassian 三处不同**：①**按产品拆 endpoint**（无聚合入口）——取文档用 Drive `https://drivemcp.googleapis.com/mcp/v1`（原预设 `workspace.googleapis.com/mcp` 是设计稿占位、不存在）；②**无 DCR**——须用户在 Cloud Console 手建「Desktop」OAuth 客户端，手动填 **client_id + client_secret**；③**redirect 不收自定义 scheme**——Desktop 客户端走 **loopback `http://127.0.0.1:<port>`**（`resound://` 不行）；④token 交换/刷新**需带 client_secret**（`client_secret_post`）；⑤需报名 Workspace Developer Preview + 启用 Drive API & Drive MCP API；scope=`drive.readonly`+`drive.file`。
- **落地**：MCPSource 加 `needsClientSecret`/`oauthScopes`（均 Optional，旧 JSON 兼容；load 合并时配置字段以预设为准防漂移）。新增 [MCPLoopbackOAuth.swift](../Sources/ResoundCore/MCPLoopbackOAuth.swift)（`LoopbackOAuthServer`：NWListener 起一次性本机 HTTP，捕获浏览器回跳的 code/校验 state；非沙盒 App 无需 entitlement）。MCPOAuth：`OAuthServerMetadata.google` 已知端点（不依赖发现）、`authorizeURL` 加 `extraParams`（Google `access_type=offline`+`prompt=consent` 拿 refresh）、`exchange`/`refresh`/`tokenRequest` 透传 client_secret、`MCPToken` 存 clientSecret 供刷新。MCPModel：`connect` 按 `needsClientSecret` 分流——Google 先弹**手动凭证录入**（`credsEntry`，client_secret 只进 Keychain）→ `runManualLoopbackOAuth`（loopback redirect + 默认浏览器开授权页 + 带 secret 换 token）；标准 DCR 流抽成 `startConnect`。新增凭证录入模态（[MCPModals.swift](../Sources/ResoundApp/MCPModals.swift) `credsModal`，附 Cloud Console 链接 + 步骤说明）。
- **追加（同日稍后）：Google 预设也移除**。用户是**企业 Google 账号**——Cloud Console 被 IT 锁,**无法自建 OAuth 客户端**(只有一个普通 API key,不能做用户 OAuth),且 Drive MCP API 需 Workspace Developer Preview 资格(企业普遍未开)。结论:Google 这条**不在用户侧能解决**,从 `builtins()` 移除该预设。**但手动凭证 + loopback 全套骨架保留**(`needsClientSecret`/[MCPLoopbackOAuth.swift](../Sources/ResoundCore/MCPLoopbackOAuth.swift)/`credsModal`/`runManualLoopbackOAuth`/`OAuthServerMetadata.google`)——dead 但功能完整,IT 配合后恢复一个预设即可。**给用户的替代路径**:企业 Drive 文档走 Resound 现成「文档导入」(PDF/Word/PPT/HTML/图片 OCR)入同一知识库(失自动同步,但内容进检索/问答/纪要);或用零配置的 Notion/Jira。内置预设现仅 **Notion + Jira/Confluence**(`.google`/`.figma` 枚举 case + 图标保留防旧 JSON 解码失败)。README 双语再同步(去 Google Drive)。**编译+打包通过、App 已重启,未 commit。**

**踩坑：取回外部文档"无权限"实为取回工具没调对（2026-06-29）**：用户连上 Notion 后粘自己的 Notion 页面链接,弹「无权访问这条内容」。**根因不是权限**,是 `SourceAdapter` 取回逻辑两处错,且错误被笼统归成 `.noPermission`:
- ① **工具名精确匹配漏了带前缀的**:Notion 取回工具叫 **`notion-fetch`**(不是 `fetch`),旧代码 `fetchToolNames.contains(name.lowercased())` 精确比对 → 匹配不到 → 抛 noContent。
- ② **参数名/键乱传**:旧代码一次性传 `url`+`uri`+`id` 三个键。查实(ToolSearch 拉真实 schema):Notion `notion-fetch` 必填 **`id`**(接受 notion.so URL);Atlassian `fetch` 是 **`additionalProperties:false`** 且 `id` 要 **ARI**(非普通 URL)——多传键会被服务端直接拒。
- **修法=按工具真实 schema 调**:`MCPClientSession.tools()` 返回每个工具的 `paramNames`/`required`(从 `Tool.inputSchema` 的 `.object` 里取 `properties`/`required` 键);`SourceAdapter.bestFetchTool` 按关键字优先级(fetch>retrieve>get_page>read>get,排除 create/update/delete/search)选工具;`argKey` 从该工具声明的参数里挑(url 类优先,其次 id 类,再退化唯一必填/唯一参数);**只传那一个键、URL 当值**。错误分流:`noFetchTool`/`noContent` → `.unknown`(非权限),其余 → `.noPermission`;真实 error 经 `AppLog` 落盘(`resound.log`)便于排查。**教训**:托管 MCP 取回工具的「名字 + 参数名 + 是否严格 schema」三家都不同,不能写死一张名单/硬塞多个键,必须读 schema 适配。**Atlassian 仍有遗留**:它 `fetch` 收 ARI 不收 URL,得先 `search` 拿 ARI 再 fetch——本次未做(用户当前测 Notion)。编译+打包+重启,待用户复测 Notion 链接取回。

**外部文档取回/显示/反馈 一组修复（2026-06-29，接上条 UI 实测）**：用户连上 Notion 实测,连环修：
- **取回正文是 JSON 一坨**：`notion-fetch` 返回的是 JSON 包裹 `{title,url,text,...}`,正文在 `text` 且带一句 "Here is the result …" 前言。`SourceAdapter.normalize` 拆包:取 `text`/`content`/`markdown`、去前言、用 JSON 里的真实 `title`(代替 URL slug)、读 `last_edited_time` 当版本戳。
- **正文渲染空白**：拆包后 `text` 是 Notion **"enhanced markdown"**——混入 `<page>/<ancestor-path>/<properties>/<table>/<details>/<span>` 等 HTML 式标签 + **tab 缩进**;原生 markdown 渲染器把标签当 HTML 丢弃、tab 当代码块 → 显示空白(但 53KB 正文确实在盘上)。新增 [NotionMarkdown.swift](../Sources/ResoundCore/NotionMarkdown.swift)`clean()`(确定性、无 LLM):删元数据块(ancestor-path/properties/colgroup)、表格 `<tr><td>`→`cell | cell` 文本行、其余标签整体去除保留内文、去前导 tab、去标题尾 `{toggle=…}`、折叠空行;`looksLikeEnhanced` 探测后才清。**原始增强格式仍留档 `original.md`**。`normalize` 里接上。
- **关联无反馈 + 列表不刷新 + 像失败**：根因=`confirmLink` 一点就秒关弹窗,`importDocument`(写盘)→`indexDocument`(embedding,几秒/可能失败)在后台静默跑;若 indexDocument 抛错则文档已写盘但 `refresh()` 不调 → 文档页能看到、录音页没刷新、像失败。改:入库期间保持弹窗显示 `.importing` 转圈;成功才关弹窗+toast+`refresh()`;失败留弹窗显示红色原因+「重试」、`AppLog` 落盘、两条路径都 `refresh()`(反映磁盘真实)。
- **文档详情标题栏按钮压住标题**：标题 `VStack` 无 `maxWidth` 约束,长标题撑满与右上角按钮重叠。改:标题区 `.frame(maxWidth:.infinity)` 内部截断、按钮组 `.fixedSize()` 不被压缩。
- **Atlassian 遗留**(未做):`fetch` 收 ARI 非 URL,得先 search 拿 ARI 再 fetch。
- **「向本文档提问」空 + 索引脏**(2026-06-29 续):查索引发现三态不一致——`documents` 有干净 id 行、`doc_links` 有干净 id、但 `chunks` 只有**旧 hex 后缀 id 的 249 条孤儿**(第一次导入标题还是 URL slug 带 hex 时留下、删除时没清),当前文档 0 chunk → doc 问答按 id 过滤命中 0 → 空。两个根因:① **外部大文档建索引太重/易失败**——35KB 切 249 chunk,默认每 chunk 一次 LLM 做 contextual enrichment=249 次 LLM,慢且脆,正是入库时 `indexDocument` 在 `setDocLinks` 后、写 chunk 前抛异常的点(documents/doc_links 已写、chunks 没写)。改:外部文档入库/同步 `enrichContext: false`(只切块+embedding;`ingestImported` 调用 + `MCPIngest.resync` 都改)。② **删文档/改 id 的索引孤儿无人清**——`Index.deleteDocument` 逻辑本身对,但更早的删除没走到、或取回后标题变更导致 id 改变留下旧 id chunks。新增 `Index.purgeOrphanDocuments(validDocIds:)`(清 chunks/documents/doc_links 里不在 vault 的文档,复用 deleteDocument 走 vec0 连接——普通 sqlite3 CLI 删不了 `chunks_vec`),`DocumentsModel.load()` 启动跑一次兜底。实测启动后 249 孤儿清净。**chunk 偏多**(249/35KB≈140 字)留意但 enrich 关掉后非瓶颈,暂不动 chunker。全部编译+打包+重启通过,未 commit。
