...
> 最近更新：2026-06-25（**开源化第一步：AI Provider 配置+实时验证+首启引导**落地，编译+打包+启动+迁移实测通过，待用户实机验收）

## 一句话现状

CLI 全链路 + macOS App 已通且用户实测过（问答 / Meet 弹窗 / 录音 / 导入 / 摘要 / 说话人识别 / VAD 门控）。**正在做开源化**：本轮把 AI 配置抽象成「可设置/可验证的 Provider + 首启引导」（见下）。**待用户实机验收。**
