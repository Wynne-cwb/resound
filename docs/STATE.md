...
> 最近更新：2026-06-26（**Onboarding 自动建 Vault**：引导/设置选文件夹即按数据契约自动创建 vault 结构（resound.yaml/people/recordings/documents/notes/.gitattributes），已是 vault 则采用；录音库设为进入主界面**必填**项。编译通过，待实机验收，**已 commit**）
