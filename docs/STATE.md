...
> 最近更新：2026-06-25（**已开源：repo 设为 public**——开源前安全体检通过、去除脚本绝对路径、英文为主双语 README、私有 vault 引用改为「从零建自己 Vault」指引。**当前转入新方向：文档模块设计**）
> 上一轮：会议自动起止录音 + 「向本场提问」+ 修两个 keep-alive bug（已 push）；再上轮：开源化 Provider 配置收尾（已 push）

## 一句话现状

CLI 全链路 + macOS App 已通且用户实测过（问答 / Meet 弹窗 / 录音 / 导入 / 摘要 / 说话人识别 / VAD 门控），**仓库已开源公开**。**当前焦点：设计「文档」大模块**——已完成 P1（地基）设计并与用户对齐，spec 待 review → 转实现计划。
...
- **📄 文档模块 P1：后端（M1）已实现 + CLI 验证通过**。spec [specs/2026-06-25-documents-p1-design.md](superpowers/specs/2026-06-25-documents-p1-design.md) / 计划 [plans/2026-06-25-documents-p1-plan.md](superpowers/plans/2026-06-25-documents-p1-plan.md)。已落地：新增 [Document.swift](../Sources/ResoundCore/Document.swift)（DocumentManifest+DocumentStore+解析/扫描）；Index 加 `source_kind`/`doc_id` 列 + `documents`/`doc_links` 表 + 文档读写 + 检索按 docId scoping；Chunker 加 `chunk(text:)` 无时间轴切块；IndexPipeline 加 `indexDocument`/`indexOneDocument` + `answerInDocument` + search 支持 docId + build 纳入 documents/；Synthesizer/CLI 引用区分 📄文档/🎙️录音；CLI 新增 `import-doc`。**CLI 验收全绿**：导入 md→纯文档问答带 📄 引用 / 旧 index 平滑迁移录音零回归 / 同一 index 真·跨源问答（一段答案 📄+🎙️ 并存）。决策见 DECISIONS 同日「文档模块 P1 后端落地」。**M2 App 视图模型也已落地（编译通过）**：新增 [DocumentsModel.swift](../Sources/ResoundApp/DocumentsModel.swift)（列表/搜索/导入文件+粘贴异步状态/编辑元数据/删除/关联录音双向/向本文档提问）+ [DocAskStore.swift](../Sources/ResoundApp/DocAskStore.swift)（doc-chats.json 持久化）+ Core 补 DocumentSummary/listDocuments/updateManifest。**未接 UI**（M3）。**下一步=M3 UI 接线**：等用户拿 spec §8.1 prompt 让 Claude Design 出图后，把 DocumentsModel 绑定到界面（文档主面/导入/详情/关联/Ask 跨源引用跳转）+ 注入 app 环境 + 加主导航入口 + 同步 data-contract。**P2 生成 / P3 富格式 / P4 在线集成 不在本期。**
