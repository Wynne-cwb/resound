...
- **📄 文档模块 P1（地基）设计已完成、待 review**，spec：[docs/superpowers/specs/2026-06-25-documents-p1-design.md](superpowers/specs/2026-06-25-documents-p1-design.md)。范围=md/txt 文档成 vault+UI 一等公民 → 自动进检索/问答（跨源引用）→ 手动关联录音（双向）。关键决策见 DECISIONS 同日「文档模块 P1 设计」（一等公民可关联录音 / 主干方案 A 边缘归一化 / 分期 P1→P4 / documents 与 notes 分实体 / chunks 加 source_kind+doc_id）。**下一步**：用户 review spec → 通过后转 writing-plans 出实现计划；UI 由用户拿 spec §8.1 的 handoff prompt 让 Claude Design 出图。**P2 生成 / P3 富格式(PDF/PPT/docx) / P4 在线集成 不在本期。**
