...
- **第二版设计稿还原(本轮,编译+打包+启动通过)**:Claude design 重做了三块 → ①**侧栏折叠按钮**改 26px 圆形(top22/right-13 跨边框)+折叠态导航 40×40 r10;②**Library 文件夹**:行内「移动到」浮层(图标→弹窗列文件夹+✓+新建)、**拖拽录音到文件夹组**(`.onDrag`/`.dropDestination`,组高亮虚线)、空文件夹占位、文件夹头 hover 出改名/删除;③**Ask 历史**:对话行加预览行(首条回答 38 字)+ hover 改名/删除、会话重命名/删除确认弹窗(改名后不被自动标题覆盖,`customTitle`)。④亮色主题色 `#bd6a2e`→**`#e85f2c`**。
- **Ask 聊天历史 + 多轮上下文(上轮,编译+打包+启动通过)**:ChatVM 升为 App 级 `@StateObject`(原来 ChatView 内 `@StateObject`,切页就丢);新增 `ChatStore` 落盘 `App Support/Resound/conversations.json`;Ask 页左加对话列表列(新对话/选中/hover 删除/相对时间);**多轮上下文**:`ChatTurn` + `renderHistory` 贯穿 `QueryPlanner.plan`(补全指代进 query)、`Synthesizer.answer`、`digestAnswer`,ChatVM 取最近 8 条历史传入 `answer(history:)`(CLI 路径默认空、不受影响)。
