...
> 最近更新：2026-06-26（**Markdown 渲染重构 + 性能**：用原生渲染器替换 MarkdownUI（swift-markdown 解析 + 自绘 + LazyVStack 虚拟化）→ 去掉 keep-alive 改条件渲染；另修本场/文档提问追问不带上下文、转录行 Equatable 化、findMatchCount 缓存。用户实测「好很多了」。埋点已关）
> 上轮更新：2026-06-26（**录音浮窗**：录音时屏幕级可拖动小药丸[脉冲红点+计时+停止]，设置「通用」加开关。用户已验收 OK）
...
- **⚡ Markdown 渲染重构 + 一组性能修复（用户实测「好很多了」，待 commit）**。设计 [specs/2026-06-26-native-markdown-renderer-design.md](superpowers/specs/2026-06-26-native-markdown-renderer-design.md)。
