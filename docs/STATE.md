...
> 最近更新：2026-07-24（**MOSS 长音频三修 + 端到端验证通过**：①>30min 自动切块并行转写（MossTranscriber，85min 实测 4 块 8.9min 出 1073 段、时间戳单调）②服务端 parse 失败按 [Sxx] 降级保底（moss_modal.py 已重部署，昨晚 39k 字失败输出验证 524 段）③在线 whisper 无 key/失败→本地 WhisperKit 兜底（不再裸打 api.openai.com 吃 401）。另修一键部署缺 fastapi（venv 补装+MossDeployer 改 pip install modal fastapi）。85min「Data Agent 对接」mp3 + 58min 抢救会议录音均已入库索引；旧路径曾把 58min 会「成功」转成只有前 35 分钟的**静默截断**版——已归档 archive/、新增 CLI `index-prune` 清索引残留。产物 build/Resound-20260724.zip。**待用户：App 里打开两条新录音验收说话人凝聚合并质量（分块标签待声纹并回）→ commit**。详见 DECISIONS 2026-07-23/24）
