...
> 最近更新：2026-07-23（**"切了 MOSS 却仍走 whisper" 的根因锁死 + Platform Weekly 已重转录**：用户反馈 07-23 Platform 周会转录满是噪音，怀疑双轨合并 bug。诊断发现：①`providers.json` 里确切了 MOSS ②但**运行中的 `build/Resound.app` 二进制 mtime=07-22 15:40:04，而 MOSS 代码 commit b8ac745=07-22 16:44:09——晚 64 分钟**，二进制里根本没有 `if cfg.mossEnabled { transcribeMoss }` 那段分支 → 静默回落 whisper 分轨 ③mic 那轨 mean_volume=**-39.2dB**（用户没说话），whisper 却对静音幻觉出 **272 段/1402s** 假话（30-40s 不换句典型幻觉形态），system 只 15 段/60s；④TranscriptMerge 只丢重叠+相似段，mic 孤立幻觉全部保留=用户看到的"噪音"。**已 killall→bundle-app.sh release→open→`resound retranscribe` 走 MOSS 混音**：248 段（无幻觉）、VAD 剪静音 103s、Mason 主讲 20.9min（占全场 84%）、认出 Mason/Tony/River 3 人熟人。**App 已 open 待用户实机验收。教训：设置页写 providers.json 数据文件立即生效，但读它的代码在二进制里——二进制不重打包，配置切了也白切**。详见 DECISIONS 2026-07-23）
> 最近更新：2026-07-22（**MOSS 已实机启用 + 58 分钟真实会议重转录验证**：用户设置里切了「MOSS 云端」；新增 CLI `retranscribe`（原地重转录：保 id/目录→MOSS→重索引→声纹命名→重摘要，老录音迁 MOSS 的通用入口）。「OS content preference 对齐」会（58min）跑通全流程：11 标签凝聚成 8 人（4 标签同人正确合并 cos 0.82–0.91）、自动认出 John/Len/Alex 3 人、GPU 推理约 35min。**待用户 App 里查看该录音验收归属质量 → 命名说话人1(210s) → 全批 commit**）
> 最近更新：2026-07-22（**MOSS 转写后端三波全量落地，编译全绿 + Wave1 CLI 端到端验证**：①Core=MossTranscriber(submit/poll)+IngestPipeline MOSS 分支(VAD→归一→MOSS→remap，失败回退 whisper)+moss-diar.json 暂存+MossSpeakers 声纹命名(60s 真录音验过：S01→Wynne cos=0.83) ②设置页转写三选「MOSS 云端★/在线/本地 Whisper」+MossDeployer 六步一键部署(venv→modal setup→secret→deploy→验证) ③Onboarding 默认推荐 MOSS+README 双语。**用户 providers.json 已预填部署好的 endpoint（backend 未切），设置里点「MOSS 云端」即启用**。App 运行中未重打包——**待用户：`killall Resound && ./scripts/bundle-app.sh release && open build/Resound.app` → 实机验收（切 MOSS→导录音→说话人自动命名→回退→重识别）→ commit**。详见 DECISIONS 2026-07-22「MOSS 转写后端全量落地」）
> 最近更新：2026-07-22（**MOSS 云端推理服务已部署 Modal 并端到端验证**：用户拍板选 MOSS（本地太卡）→ 部署到 Modal L4（每月 $30 免费额度，绑卡解锁）。异步 submit/poll 两个 endpoint，16min 录音 RTF 0.36、输出与本地 MLX 一致，成本约 $9/月在免费额内。代码 [moss_modal.py](../experiments/moss-eval/moss_modal.py)，key 在 Modal Secret + 本地 `.moss-api-key`（gitignored）。**下一步：待用户确认后把它接进 Resound 当转写 backend（submit/poll 进 ResoundCore + providers 配置），识人仍走本地 CAM++**。4 个 Modal 踩坑见 DECISIONS 2026-07-22）
> 最近更新：2026-07-22（**MOSS-Transcribe-Diarize 0.9B 评测完成**：端到端转录+说话人联合模型，两条真实录音实测——多人会说话人归属明显强于现管线（争论/问答轮次拆对、抓到被吞的短附和）、难音频转录更通顺；弱点=中英夹杂术语裸输出差（热词只在精简列表时部分有效，兜底靠现有校对层）+ 无声纹 embedding（跨录音识人仍需 CAM++）+ 无词级时间戳（逐句点跳会变粗）。RTF≈0.5 纯本地。报告 `experiments/moss-eval/REPORT.md`（gitignored）。**建议影子模式接入，待用户拍板 + 抽听 4 处归属分歧**。详见 DECISIONS 2026-07-22）
> 最近更新：2026-07-20（**修 Notion(MCP)token 每次弹 Keychain 密码框**：根因非密码难记，是 login keychain 的 item 带 per-app ACL + `MCPTokenStore.save` 每次 refresh 删重建 item 重置授权 + 早期 ad-hoc/CLI 污染 ACL。方案 A（用户拍板）=给 token item 附「本机任意 app 无提示访问」的开放 ACL：新增 [MCPOAuth.swift](../Sources/ResoundCore/MCPOAuth.swift) `MCPTokenStore.openAccess()`（SecAccessCreate + 逐 ACL `SecACLSetContents(nil, …, prompt=0)`），save 加 `kSecAttrAccess`。安全取舍：同机进程可读该 token（个人 Mac 风险低）。编译+打包（Resound Dev 签名）完成。**待用户：打开新 App→设置›外部 MCP 把 Notion 断开重连一次（旧污染 item 借 delete+add 重建为开放 ACL）→之后重 build/重启不再弹。README 无需改（"tokens live in Keychain" 仍成立）。待 commit。** 详见 DECISIONS 2026-07-20）
> 最近更新：2026-07-09（**录音合并功能**：列表多选 2+ 条→合并成一条（按创建时间首尾相接）→复用 `ingestOne` 重跑完整转录/索引/说话人/摘要；新标题 flash 建议可改；原录音移到 `<vault>/archive/`（可恢复不删）。新增 [AudioMerge.swift](../Sources/ResoundCore/AudioMerge.swift) + [VaultBrowser.archiveRecording](../Sources/ResoundCore/VaultBrowser.swift)；LibraryModel 多选/合并状态+`beginMerge`/`confirmMerge`；LibraryView 头部开关+行勾选框+底部 mergeBar；Overlays `mergeRecModal`。归档只在 ingest 成功后做、失败原录音不动可重试。编译+打包+重启+README 双语同步。详见 DECISIONS 2026-07-09。**待实机验收+commit**）
> 最近更新：2026-07-09（**单录音提问纳入关联文档**：向录音提问时不再漏掉挂在它上面的文档。`answerInRecording` 原严格按 recordingId 过滤→关联文档 chunk（recording_id=NULL）被整体排除。修：[Index.Filters](../Sources/ResoundCore/Index.swift) 加 `linkedDocIds` OR-scope（`recording_id=? or doc_id in(…)`）+ `answerInRecording` 用 `documentsLinked(toRecording:)` 反查传入；App 引用层 RecCite/StoredRecCite 加 docId/docTitle、[recCitesView](../Sources/ResoundApp/LibraryView.swift) 文档命中渲染蓝色文档卡+点击跳 Documents 页。DB 验证候选 27→56。编译+打包+重启+README 双语同步完成。详见 DECISIONS 2026-07-09。**待实机验收+commit**）
