...
> 最近更新：2026-06-29（**内置来源精简至 Notion + Jira/Confluence**：Figma 移除（服务端对第三方封闭）、Google 移除（企业账号无法自建 OAuth 客户端 + 无 Developer Preview 资格）；手动凭证 + loopback OAuth 骨架保留待用。编译+打包通过、App 已重启。详见 [DECISIONS](DECISIONS.md) 2026-06-29 条目）
...
文档模块 P1-M3 + P2 + P3 + 智能分类（`0330a5b`）等早前工作已 commit+push。**MCP 双向接入 Wave 0–4 全部代码 + bundle 脚本改动 + README/STATE/DECISIONS 同步均未 commit**（按规矩等用户发话）。
