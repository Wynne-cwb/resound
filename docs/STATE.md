...
> 最近更新：2026-06-25（5 项 UI/逻辑小优化落地：窗口拖动/查找替换大小写/说话人配色排序/摘要并发状态/文件夹折叠持久化，编译+打包+启动通过，待用户实机验收）

## 一句话现状

CLI 全链路 + macOS App 已通且用户实测过（问答 / Meet 弹窗 / 录音 / 导入 / 摘要 / 说话人识别 / VAD 门控）。本轮做了 5 项体验小优化（见下）。**待用户实机验收。**
