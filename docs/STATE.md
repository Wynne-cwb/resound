...
> 最近更新：2026-06-26（**智能推算文件夹/Tag 已实机验收 OK**：路径 2 独立分类器 `AutoClassifier` + 调试 CLI + 派生存储 `SuggestionStore` + 列表角标 `.popover` 确认 + 入库自动算 + 右键重算（带「推算中…」转圈反馈）。修了 3 个验收问题：重算无角标→显示判据改「建议≠当前文件夹」、浮层遮盖→改原生 popover、tag 偶发空→prompt 收紧+空结果重试一次。待 commit+push）
...
- **✅ 智能推算文件夹/Tag（已实机验收 OK，待 commit）**。spec [specs/2026-06-26-auto-classify-folder-tag-design.md](superpowers/specs/2026-06-26-auto-classify-folder-tag-design.md) / 计划 [plans/2026-06-26-auto-classify-folder-tag-plan.md](superpowers/plans/2026-06-26-auto-classify-folder-tag-plan.md)（已 commit `c43299d`/`41ba814`）。共识：建议+确认、优先复用现有可提新建、入库自动+重新推算、录音 1 文件夹 / 文档 1-2 tag、**列表角标**呈现、路径 2 独立分类器。
